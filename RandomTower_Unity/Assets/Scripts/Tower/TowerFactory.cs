...
public class TowerFactory
{
    ...
    public ITower CreateTowerByData(TowerData data, IEnemyProvider enemyProvider)
    {
    ...
    private Pool<BaseTower> GetTowerPool(TowerData data)
    {
        ...
        pool = new Pool<BaseTower>(data.Prefab, _towerGroup);
        _pools[data.ID] = pool;
    ...
    public void Return(ITower tower)
    {
        if(tower is BaseTower baseTower &&
            _pools.TryGetValue(baseTower.ID, out Pool<BaseTower> pool))
        {
